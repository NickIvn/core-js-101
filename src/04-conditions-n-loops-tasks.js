...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${leftBracket}${sortedNumbers[0]}, ${sortedNumbers[1]}${rightBracket}`;
}
...
function getDigitalRoot(num) {
  ...
  const arr = sum.toString().split('').map(Number);
...
function evaluateTicTacToePosition(position) {
  ...
  const winnerCombo = winningCombinations.find((combo) => {
    const values = combo.map(([row, col]) => position[row][col]);
    return values[0] && values.every((val) => val === values[0]);
  });
  if (winnerCombo) {
    return position[winnerCombo[0][0]][winnerCombo[0][1]];
  }
