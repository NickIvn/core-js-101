...
function concatenateStrings(value1, value2) {
  const res = value1.concat(value2);
  return res;
}
...
function getRectangleString(width, height) {
  ...
  const middleRows = (`│${' '.repeat(width - 2)}│\n`).repeat(height - 2);
  const verticalBorder = `└${'─'.repeat(width - 2)}┘\n`;

  return horizontalBorder + middleRows + verticalBorder;
}
...
function encodeToRot13(str) {
  ...
  const encript = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  return str.replace(/[a-z]/gi, (letter) => encript[origin.indexOf(letter)]);
}
